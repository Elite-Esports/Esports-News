...
from keys import bearer_token, openai_api_key, LANGCHAIN_API_KEY
import csv
import json
...
app = FastAPI()  # Create instance of application
...
async def get_stories():
    try:
        return FileResponse("stories.json", media_type="application/json")
...
async def generate_stories_endpoint():
    ...
    api = tweepy.Client(bearer_token)  #use bearer token to connect to the application on the dev account
    ...
    def clear_json_file(file):
        """Erase the JSON file before starting to add a group of stories."""
        with open(file, 'w') as file:
            json.dump([], file)

    
  
    clear_json_file("stories.json")
    ...
    - article: A detailed and engaging article using specific tweets with the same topic. Ensure the article captures the context, relevance, and impact of the specific information presented in the tweets.
    - media: If any tweet used for an article contains a link in the 'media' field, include it in this field
    - usernames: the usernames where you obtained the media fields
    ...
    for username in usernames:
        if username:
            search_query = f"from:{username} -is:reply -is:retweet"  
